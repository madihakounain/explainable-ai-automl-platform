...
function predict() {
  ...
  .then(data => {
    ...
    const limePlaceholder = document.getElementById('lime-placeholder');

    if (data.shap_chart) {
      const shapImg = document.getElementById('shap-chart-img');
      shapImg.src = 'data:image/png;base64,' + data.shap_chart;
      shapImg.classList.remove('hidden');
      shapPlaceholder.classList.add('hidden');
    } else {
      shapPlaceholder.textContent = '⚠️ SHAP chart could not be generated for this prediction.';
      shapPlaceholder.classList.remove('hidden');
    }

    if (data.lime_chart) {
      const limeImg = document.getElementById('lime-chart-img');
      limeImg.src = 'data:image/png;base64,' + data.lime_chart;
      limeImg.classList.remove('hidden');
      limePlaceholder.classList.add('hidden');
    } else {
      limePlaceholder.textContent = '⚠️ LIME chart could not be generated for this prediction.';
...
function switchXaiTab(name, btn) {
